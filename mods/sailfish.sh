...
if [[ $(NGSANE_CHECKPOINT_TASK) == "start" ]]; then
    ...
    -l $SAILFISHLIBRARY \
    -1 <(gunzip -c $f) \
    -2 <(gunzip -c ${f/%$READONE.$FASTQ/$READTWO.$FASTQ})
    
    # mark checkpoint
     NGSANE_CHECKPOINT_CHECK  $OUTDIR/quant_bias_corrected.sf $OUTDIR/quant.sf 
fi
################################################################################
...
if [[ $(NGSANE_CHECKPOINT_TASK) == "start" ]]; then
    ...
    if [ ! -f $OUTDIR/"$SAMPLE"_quant.sf_cut ]; then
        sed '1,5d' $OUTDIR/quant.sf > $OUTDIR/"$SAMPLE"_quant.sf_cut; else echo " Couldn't delete first 5 lines of quant_sf."
    fi
    ...
    if [ ! -f $OUTDIR/"$SAMPLE"_quant_bias_corrected.sf_cut ]; then
        sed '1,5d' $OUTDIR/quant_bias_corrected.sf > $OUTDIR/"$SAMPLE"_quant_bias_corrected.sf_cut; else echo " Couldn't delete first 5 lines of bias_sf."
    fi
